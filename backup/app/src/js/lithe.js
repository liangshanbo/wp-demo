	...
	if (isBrowser) {
		...
		var loadPublicDeps = function(cb) {
			if (lithe.publicpath && publicDeps.length) {
				...
				LEVENTS.trigger('fetch', [nginxPublic, cb]);
			}else{
				cb();
			}
		};
			...
			use: function(urls, cb) {
				if(!CONFIG || isInitConfig){
					...
					realUse(CONFIG, function(cg) {
						setConfig(cg);
						if (lithe.config.preload && lithe.config.preload.length) {
							realUse(lithe.config.preload, function(){
								lithe.config.preload = [];
								realUse(urls, cb);
							});
						}else{
